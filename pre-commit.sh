...
if ! git diff --cached --quiet -- coroutine; then (
    ...
    cargo +1.91.1 clippy --all-features --all-targets --locked --workspace -- -D warnings
    cargo +1.91.1 test --locked --workspace
) fi
...
if ! git diff --cached --quiet -- structured_concurrency; then (
    ...
    cargo +1.91.1 clippy --all-features --all-targets --locked -- -D warnings
    cargo +1.91.1 check --locked
) fi
