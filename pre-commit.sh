...
if ! git diff --cached --quiet -- coroutine; then (
    ...
    cargo +1.86.0 clippy --all-features --all-targets --locked --workspace -- -D warnings
    cargo +1.86.0 test --locked --workspace
) fi
...
if ! git diff --cached --quiet -- structured_concurrency; then (
    ...
    cargo +1.86.0 clippy --all-features --all-targets --locked -- -D warnings
